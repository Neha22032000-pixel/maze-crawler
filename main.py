...
def choose_build(config, energy, counts, crystal_count, node_count):
    ...
    if scouts < 2 and energy >= get_cfg(config, "scoutCost", 50) + 450:
        return "BUILD_SCOUT"
    if crystal_count and scouts < 5 and energy >= get_cfg(config, "scoutCost", 50) + 550:
        return "BUILD_SCOUT"
    return None
...
def agent(obs, config):
    ...
    for uid, data in mobiles:
        ...
        current_wall = wall_at(obs, config, col, row)
        if rtype == 2 and (current_wall & NORTH) and energy >= get_cfg(config, "wallRemoveCost", 100) + 80:
